...
- planned-vs-executed input details for failed conditional jobs,
- concrete skip reasons for skipped jobs,
...
When using `--up` or `--up-to-layer`, coverage applies only to scheduled layers. Do not treat later-layer condition fields as missing if those layers were intentionally excluded.
