...
This file is the source map and guide index for the per-model prompt references in this skill.
...
- Use the concrete guide files in this folder first when they exist. Use this file when you need to trace or refresh the underlying sources.
...
- FLUX.2 naming is moving quickly across variants. When we create the per-model guides, we should double-check which exact variants the skill should support and name the files accordingly.
