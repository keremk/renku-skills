...
Use this template when creating a per-model prompt guide for the `create-video` skill.

This is not the prompt guide itself. It is the authoring template for building one guide per model in a consistent format.
...
This section is specifically for the `create-video` skill.
